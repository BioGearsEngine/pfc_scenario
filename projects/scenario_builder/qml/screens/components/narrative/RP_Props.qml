...
ColumnLayout {
  ...
  StackLayout {
    ...
    //           return
    //         }
    //         role_stack.currentIndex = 0 full_listArea.model.clear()root.backend.roles()while (root.backend.next_role(self)) {
    ...
    //         }
    //         self.role_id = -1 self.name = "New Role %1".arg(next)self.description = "Description of Role %1".arg(next)self_scene.scene_id = root.model.get(root.index).id self_scene.name = root.model.get(root.index).name while (root.backend.select_role(self)) {
    //           next = next + 1
    //           self.role_id = -1;
    //           self.name = "New Role %1".arg(next);
    //           self.description = "Description of Role %1".arg(next)
    ...
    //         root.backend.update_role_in_scene(self_scene, self)listArea.model.insert(listArea.model.count, {
    //           "id": self.role_id,
    //           "name": "%1".arg(self.name),
    //           "description": "%1".arg(self.description)
    //         });
    ...
    //           return
    //         }
    //         self.role_id = listArea.model.get(listArea.currentIndex).role_id self.name = listArea.model.get(listArea.currentIndex).name self_scene.scene_id = root.model.get(root.index).id self_scene.name = root.model.get(root.index).name root.backend.remove_role_from_scene(self, self_scene)listArea.model.remove(listArea.currentIndex)listArea.currentIndex = Math.max(0, listArea.currentIndex - 1)
    //       }
    //     }
    ...
    //           }
    //           onDoubleClicked : {
    //             roleEdit.name = listArea.model.get(index).name roleEdit.description = listArea.model.get(index).description roleEdit.roleID = listArea.model.get(index).id contentStack.currentIndex = 1
    //           }
    //         }
    ...
    //         root.backend.roles()while (root.backend.next_role(self)) {
  ...
  onIndexChanged : {
    ...
    if (values && model.count != 0) {
      objects_in_scene.model.clear()

      self_scene.scene_id = root.model.get(root.index).id
      self_scene.name = root.model.get(root.index).name
      root.backend.equipment_in_scene(self_scene)

      while (root.backend.next_equipment(self_equipment)) {
        listArea.model.insert(listArea.model.count, {
          "id": self.role_id,
          "name": "%1".arg(self.name),
          "description": "%1".arg(self.description)
